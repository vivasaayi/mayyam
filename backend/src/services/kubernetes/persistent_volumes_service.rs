...
impl PersistentVolumesService {
    ...
    pub async fn list_persistent_volumes(
        ...
        let pv_list = api.list(&lp).await.map_err(|e| {
            AppError::ExternalService(format!("Failed to list PVs: {}", e))
        })?;
        ...
        for pv in pv_list {
            ...
            let pv_status = status.and_then(|s| s.phase.clone()).unwrap_or_else(|| "Unknown".to_string());
            let reason = status.and_then(|s| s.reason.clone()).unwrap_or_else(|| "".to_string());
            
            let claim_ref = spec.and_then(|s| s.claim_ref.as_ref());
            let claim = claim_ref.map_or_else(
                || "-".to_string(), 
                |cr| format!("{}/{}", cr.namespace.as_deref().unwrap_or(""), cr.name.as_deref().unwrap_or(""))
            );

            let storage_class = spec.and_then(|s| s.storage_class_name.clone()).unwrap_or_else(|| "-".to_string());
    ...
    pub async fn get_persistent_volume_details(
        ...
        api.get(name).await.map_err(|e| {
            AppError::ExternalService(format!("Failed to get PV '{}': {}", name, e))
        })
    }
}
