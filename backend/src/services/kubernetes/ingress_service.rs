...
impl IngressService {
    ...
    async fn api(cluster: &KubernetesClusterConfig, namespace: &str) -> Result<Api<Ingress>, AppError> {
    ...
    pub async fn list(&self, cluster: &KubernetesClusterConfig, namespace: &str) -> Result<Vec<Ingress>, AppError> {
    ...
    pub async fn get(&self, cluster: &KubernetesClusterConfig, namespace: &str, name: &str) -> Result<Ingress, AppError> {
        let api: Api<Ingress> = Api::namespaced(ClientFactory::get_client(cluster).await?, namespace);
        api.get(name).await.map_err(|e| AppError::Kubernetes(e.to_string()))
    }

    pub async fn upsert(&self, cluster: &KubernetesClusterConfig, namespace: &str, item: &Ingress) -> Result<Ingress, AppError> {
        let api: Api<Ingress> = Api::namespaced(ClientFactory::get_client(cluster).await?, namespace);
        let params = PatchParams::apply("mayyam").force();
        api.patch(item.metadata.name.as_ref().ok_or_else(|| AppError::BadRequest("Ingress.metadata.name required".into()))?, &params, &Patch::Apply(item))
            .await
