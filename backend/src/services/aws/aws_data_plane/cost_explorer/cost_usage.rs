...
impl CostAndUsage for AwsCostService {
    ...
    async fn compare_costs(
        ...
        let result1 = self
            .get_cost_for_preset(aws_account_dto, date1.clone(), group_by.clone())
            .await?;
        let result2 = self
            .get_cost_for_preset(aws_account_dto, date2.clone(), group_by.clone())
            .await?;
...
fn get_preset_dates(preset: DatePreset) -> Result<(String, String), AppError> {
    ...
    match preset {
        ...
        DatePreset::LastMonth => {
            ...
            let end = last_month
                .with_day(days_in_month(last_month.year(), last_month.month()) as u32)
                .unwrap();
            Ok((
                start.format("%Y-%m-%d").to_string(),
                end.format("%Y-%m-%d").to_string(),
