...
use mayyam::controllers::kafka::{
    ...
    get_kafka_backup_inventory_pillar_reports, get_kafka_broker_disk_inventory_pillar_reports,
    ...
    get_kafka_lag_inventory_pillar_reports, get_kafka_managed_service_inventory_pillar_reports,
    get_kafka_message_replay_inventory_pillar_reports,
    get_kafka_mirror_maker_inventory_pillar_reports,
    get_kafka_network_throughput_inventory_pillar_reports,
...
async fn kafka_cluster_inventory_pillar_reports_contract() {
    ...
    let app = test::init_service(
        App::new()
            ...
            .route(
                "/api/kafka/inventory/broker-disk/pillars",
                web::get().to(get_kafka_broker_disk_inventory_pillar_reports),
            )
            .route(
                "/api/kafka/inventory/network-throughput/pillars",
                web::get().to(get_kafka_network_throughput_inventory_pillar_reports),
            )
            .route(
                "/api/kafka/inventory/managed-services/pillars",
                web::get().to(get_kafka_managed_service_inventory_pillar_reports),
            ),
    )
    .await;
    ...
    let request = test::TestRequest::get()
        .uri("/api/kafka/inventory/broker-disk/pillars")
        .to_request();
    let response = test::call_service(&app, request).await;
    assert_eq!(response.status(), StatusCode::OK);

    let body: Value = test::read_body_json(response).await;
    assert_eq!(body["resource_type"], "KafkaBrokerDisk");
    assert_eq!(body["resources_evaluated"], 1);
    let reports = body["reports"].as_array().expect("reports array");
    assert_eq!(reports.len(), 3);
    assert_eq!(reports[0]["pillar"], "cost");
    ...
    let request = test::TestRequest::get()
        .uri("/api/kafka/inventory/broker-disk/pillars?pillar=resilience,security")
        .to_request();
    let response = test::call_service(&app, request).await;
    assert_eq!(response.status(), StatusCode::OK);
    let body: Value = test::read_body_json(response).await;
    ...
    let request = test::TestRequest::get()
        .uri("/api/kafka/inventory/broker-disk/pillars?pillar=bogus")
        .to_request();
    let response = test::call_service(&app, request).await;
    assert_eq!(response.status(), StatusCode::BAD_REQUEST);

    let request = test::TestRequest::get()
        .uri("/api/kafka/inventory/network-throughput/pillars")
        .to_request();
    let response = test::call_service(&app, request).await;
    assert_eq!(response.status(), StatusCode::OK);

    let body: Value = test::read_body_json(response).await;
    assert_eq!(body["resource_type"], "KafkaNetworkThroughput");
    assert_eq!(body["resources_evaluated"], 1);
    let reports = body["reports"].as_array().expect("reports array");
    assert_eq!(reports.len(), 3);
    assert_eq!(reports[0]["pillar"], "cost");
    ...
    let request = test::TestRequest::get()
        .uri("/api/kafka/inventory/network-throughput/pillars?pillar=cost,security")
        .to_request();
    let response = test::call_service(&app, request).await;
    assert_eq!(response.status(), StatusCode::OK);
    let body: Value = test::read_body_json(response).await;
    ...
    let request = test::TestRequest::get()
        .uri("/api/kafka/inventory/network-throughput/pillars?pillar=bogus")
        .to_request();
    let response = test::call_service(&app, request).await;
    assert_eq!(response.status(), StatusCode::BAD_REQUEST);

    let request = test::TestRequest::get()
        .uri("/api/kafka/inventory/managed-services/pillars")
        .to_request();
    let response = test::call_service(&app, request).await;
    assert_eq!(response.status(), StatusCode::OK);

    let body: Value = test::read_body_json(response).await;
    assert_eq!(body["resource_type"], "KafkaManagedService");
    assert_eq!(body["resources_evaluated"], 1);
    let reports = body["reports"].as_array().expect("reports array");
    assert_eq!(reports.len(), 3);
    assert_eq!(reports[0]["pillar"], "cost");
    ...
    let request = test::TestRequest::get()
        .uri("/api/kafka/inventory/managed-services/pillars?pillar=resilience,security")
        .to_request();
    let response = test::call_service(&app, request).await;
    assert_eq!(response.status(), StatusCode::OK);
    let body: Value = test::read_body_json(response).await;
    ...
    let request = test::TestRequest::get()
        .uri("/api/kafka/inventory/managed-services/pillars?pillar=bogus")
        .to_request();
    let response = test::call_service(&app, request).await;
    assert_eq!(response.status(), StatusCode::BAD_REQUEST);
}
