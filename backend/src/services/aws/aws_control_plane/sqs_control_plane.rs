...
impl SqsControlPlane {
    ...
    pub async fn sync_queues(&self, account_id: &str, profile: &AwsAccountDto, region: &str) -> Result<Vec<AwsResourceModel>, AppError> {
        self.sync_queues_with_auth(account_id, profile, region, None).await
    }

    pub async fn sync_queues_with_auth(&self, account_id: &str, profile: &AwsAccountDto, region: &str, account_auth: Option<&AccountAuthInfo>) -> Result<Vec<AwsResourceModel>, AppError> {
        let client = self.aws_service.create_sqs_client_with_auth(profile, region).await?;
        self.sync_queues_with_client(account_id, profile, region, client).await
    }

    async fn sync_queues_with_client(&self, account_id: &str, profile: &AwsAccountDto, region: &str, client: SqsClient) -> Result<Vec<AwsResourceModel>, AppError> {
