...
impl ExplainPlanService {
    ...
    pub async fn execute_explain_plan(
        ...
        let (query, format) = match cluster.engine.to_lowercase().as_str() {
            "mysql" | "aurora-mysql" => (
                format!("EXPLAIN FORMAT=JSON {}", fingerprint.normalized_sql),
                "JSON",
            ),
            _ => (
                format!(
        ...
        let plan_json: serde_json::Value =
            serde_json::from_str(&plan_data).unwrap_or(serde_json::Value::Null);
        ...
        self.update_plan_analysis(created_plan.id, &analysis)
            .await?;
    ...
    pub fn analyze_explain_plan(&self, plan: &ExplainPlan) -> Result<ExplainPlanAnalysis, String> {
        ...
        match plan.plan_format.as_str() {
            ...
            "TRADITIONAL" => self.analyze_traditional_plan(
                &serde_json::Value::String(plan.plan_data.clone()),
                &mut analysis,
            )?,
            _ => {}
        }
    ...
    fn analyze_mysql_json_plan(
        &self,
        plan_data: &serde_json::Value,
        analysis: &mut ExplainPlanAnalysis,
    ) -> Result<(), String> {
    ...
    fn analyze_mysql_query_block(
        &self,
        query_block: &serde_json::Value,
        analysis: &mut ExplainPlanAnalysis,
    ) -> Result<(), String> {
    ...
    fn analyze_postgresql_json_plan(
        &self,
        plan_data: &serde_json::Value,
        analysis: &mut ExplainPlanAnalysis,
    ) -> Result<(), String> {
    ...
    fn analyze_traditional_plan(
        &self,
        _plan_data: &serde_json::Value,
        _analysis: &mut ExplainPlanAnalysis,
    ) -> Result<(), String> {
        // Placeholder for legacy support
        Ok(())
    }

    async fn update_plan_analysis(
        &self,
        plan_id: Uuid,
        analysis: &ExplainPlanAnalysis,
    ) -> Result<(), String> {
        self.explain_repo
            .update_optimization_flags(
                plan_id,
    ...
    pub async fn get_plan_recommendations(&self, plan_id: Uuid) -> Result<Vec<String>, String> {
        ...
        if analysis.has_filesort {
            recommendations.push("Filesort detected - add indexes on ORDER BY columns".to_string());
        }
        if analysis.has_temp_table {
            recommendations
                .push("Temporary table created - review GROUP BY performance".to_string());
        }
        Ok(recommendations)
    }
    ...
    pub async fn compare_explain_plans(
        ...
        let plan_1 = self
            .explain_repo
            .find_by_id(plan_id_1)
            .await?
            .ok_or("Plan 1 not found")?;
        let plan_2 = self
            .explain_repo
            .find_by_id(plan_id_2)
            .await?
            .ok_or("Plan 2 not found")?;
        Ok(PlanComparison {
            plan_1,
            plan_2,
    ...
    pub async fn update_optimization_flags(
        ...
        self.explain_repo
            .update_optimization_flags(plan_id, idx, fs, fsort, tmp)
            .await
    }
}
