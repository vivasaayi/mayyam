...
pub async fn list_service_accounts_controller(
    claims: web::ReqData<Claims>,
    db: web::Data<Arc<DatabaseConnection>>,
    path: web::Path<(String, String)>,
    svc: web::Data<Arc<ServiceAccountsService>>,
) -> Result<impl Responder, AppError> {
    let (cluster_id, ns) = path.into_inner();
    debug!(target: "mayyam::controllers::service_accounts", user_id = %claims.username, %cluster_id, %ns, "List ServiceAccounts");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
    let list = svc.list(&cfg, &ns).await?;
    Ok(HttpResponse::Ok().json(list))
}

pub async fn get_service_account_controller(
    claims: web::ReqData<Claims>,
    db: web::Data<Arc<DatabaseConnection>>,
    ...
    debug!(target: "mayyam::controllers::service_accounts", user_id = %claims.username, %cluster_id, %ns, %name, "Get ServiceAccount");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
    let item = svc.get(&cfg, &ns, &name).await?;
    Ok(HttpResponse::Ok().json(item))
}

pub async fn upsert_service_account_controller(
    claims: web::ReqData<Claims>,
    db: web::Data<Arc<DatabaseConnection>>,
    path: web::Path<(String, String, String)>,
    body: web::Json<ServiceAccount>,
    svc: web::Data<Arc<ServiceAccountsService>>,
) -> Result<impl Responder, AppError> {
    let (cluster_id, ns, name) = path.into_inner();
    let mut item = body.into_inner();
    if item.metadata.name.as_deref() != Some(&name) { item.metadata.name = Some(name.clone()); }
    debug!(target: "mayyam::controllers::service_accounts", user_id = %claims.username, %cluster_id, %ns, %name, "Upsert ServiceAccount");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
    let updated = svc.upsert(&cfg, &ns, &item).await?;
    Ok(HttpResponse::Ok().json(updated))
}

pub async fn delete_service_account_controller(
    claims: web::ReqData<Claims>,
    db: web::Data<Arc<DatabaseConnection>>,
    path: web::Path<(String, String, String)>,
    svc: web::Data<Arc<ServiceAccountsService>>,
    ...
    let (cluster_id, ns, name) = path.into_inner();
    debug!(target: "mayyam::controllers::service_accounts", user_id = %claims.username, %cluster_id, %ns, %name, "Delete ServiceAccount");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
