...
pub async fn list_replica_sets_controller(
    ...
    let replica_sets = rs_service
        .list_replica_sets(&cluster_config, &namespace_name)
        .await?;
...
pub async fn get_replica_set_controller(
    ...
    let cluster_config = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
    let details = rs_service
        .get_replica_set_details(&cluster_config, &namespace_name, &rs_name)
        .await?;
