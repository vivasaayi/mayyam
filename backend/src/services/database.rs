...
impl DatabaseService {
    ...
    pub async fn test_connection(&self, conn_model: &crate::models::database::Model) -> Result<ConnectionTestResult, AppError> {
        use crate::utils::database::{test_database_connection, connect_to_dynamic_database};
        ...
        let result = match conn_model.connection_type.as_str() {
            "mysql" => {
                ...
                let _stats = ConnectionTrait::query_one(&conn, Statement::from_string(
                    DbBackend::MySql,
                    "SHOW STATUS LIKE 'Max_used_connections'".to_string()
                )).await.map_err(AppError::Database)?;
                
                let _version_result = ConnectionTrait::query_one(&conn, Statement::from_string(
                    DbBackend::MySql,
    ...
    pub async fn get_schema(&self, conn_model: &crate::models::database::Model) -> Result<serde_json::Value, AppError> {
        ...
        match conn_model.connection_type.as_str() {
            "mysql" => {
                ...
                let _tables_result = ConnectionTrait::query_all(&conn, Statement::from_string(
                    DbBackend::MySql,
                ...
                for _table_row in _tables_result {
                    ...
                    let _columns_result = ConnectionTrait::query_all(&conn, Statement::from_string(
                        DbBackend::MySql,
                        columns_query
                    )).await.map_err(AppError::Database)?;
