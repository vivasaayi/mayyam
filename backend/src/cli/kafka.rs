...
pub async fn handle_command(command: KafkaCommands, config: &Config) -> Result<(), Box<dyn Error>> {
    match command {
        ...
        KafkaCommands::Connect { name } => {
            ...
            let cluster = config.kafka.clusters.iter()
                .find(|c| c.name == name)
        ...
        KafkaCommands::CreateTopic { cluster, topic, partitions, replication } => {
            ...
            // Find the cluster configuration
            let kafka_cluster = config.kafka.clusters.iter()
                .find(|c| c.name == cluster)
                .ok_or_else(|| format!("Kafka cluster '{}' not found in configuration", cluster))?;
            
            // Build a client configuration
            let mut client_config = ClientConfig::new();
            client_config.set("bootstrap.servers", &kafka_cluster.bootstrap_servers.join(","));
            client_config.set("client.id", "mayyam-cli");
            
            // Add security configuration if present
            if let (Some(username), Some(password)) = (&kafka_cluster.sasl_username, &kafka_cluster.sasl_password) {
                client_config.set("sasl.username", username);
                client_config.set("sasl.password", password);
