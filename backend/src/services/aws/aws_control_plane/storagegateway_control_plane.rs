...
pub struct StorageGatewayControlPlane {
    aws_service: Arc<AwsService>,
}

impl StorageGatewayControlPlane {
    ...
    pub async fn sync_gateways(
        ...
        debug!(
            "Syncing Storage Gateways for account: {} with sync_id: {}",
        ...
        loop {
            let mut request = client.list_gateways();
            ...
            let response = match request.send().await {
                Ok(res) => res,
                Err(e) => {
                    error!("Failed to list Storage Gateways: {}", e);
                    break;
                }
            };
            ...
            for gw in response.gateways() {
                ...
                let dto = AwsResourceDto {
                    ...
                    resource_type: AwsResourceType::StorageGateway.to_string(),
                    resource_id: gw_id.to_string(),
                    arn: arn.to_string(),
        ...
        debug!(
            "Successfully synced {} Storage Gateways for account: {} with sync_id: {}",
            resources.len(),
