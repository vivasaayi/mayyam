...
impl ServiceAccountsService {
    ...
    async fn api(cluster: &KubernetesClusterConfig, namespace: &str) -> Result<Api<ServiceAccount>, AppError> {
    ...
    pub async fn list(&self, cluster: &KubernetesClusterConfig, namespace: &str) -> Result<Vec<ServiceAccount>, AppError> {
    ...
    pub async fn get(&self, cluster: &KubernetesClusterConfig, namespace: &str, name: &str) -> Result<ServiceAccount, AppError> {
        let api: Api<ServiceAccount> = Api::namespaced(ClientFactory::get_client(cluster).await?, namespace);
        api.get(name).await.map_err(|e| AppError::Kubernetes(e.to_string()))
    }

    pub async fn upsert(&self, cluster: &KubernetesClusterConfig, namespace: &str, item: &ServiceAccount) -> Result<ServiceAccount, AppError> {
        let api: Api<ServiceAccount> = Api::namespaced(ClientFactory::get_client(cluster).await?, namespace);
        let params = PatchParams::apply("mayyam").force();
        api.patch(item.metadata.name.as_ref().ok_or_else(|| AppError::BadRequest("ServiceAccount.metadata.name required".into()))?, &params, &Patch::Apply(item))
            .await
