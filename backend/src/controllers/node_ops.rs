...
pub struct TaintRequest { pub key: String, pub value: String, pub effect: String }
...
pub async fn cordon_node_controller(
    claims: web::ReqData<Claims>, db: web::Data<Arc<DatabaseConnection>>, path: web::Path<(String, String)>, svc: web::Data<Arc<NodeOpsService>>,
...
pub async fn add_taint_controller(
    claims: web::ReqData<Claims>, db: web::Data<Arc<DatabaseConnection>>, path: web::Path<(String, String)>, body: web::Json<TaintRequest>, svc: web::Data<Arc<NodeOpsService>>,
) -> Result<impl Responder, AppError> {
    let (cluster_id, node) = path.into_inner();
    ...
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
    let updated = svc.add_taint(&cfg, &node, &req.key, &req.value, &req.effect).await?;
    Ok(HttpResponse::Ok().json(updated))
}

#[derive(serde::Deserialize)]
pub struct RemoveTaintRequest { pub key: String }

pub async fn remove_taint_controller(
    claims: web::ReqData<Claims>, db: web::Data<Arc<DatabaseConnection>>, path: web::Path<(String, String)>, body: web::Json<RemoveTaintRequest>, svc: web::Data<Arc<NodeOpsService>>,
) -> Result<impl Responder, AppError> {
    let (cluster_id, node) = path.into_inner();
    let req = body.into_inner();
