...
impl PersistentVolumeClaimsService {
    ...
    pub async fn list_persistent_volume_claims(
        ...
        let pvc_list = api.list(&lp).await.map_err(|e| {
            AppError::ExternalService(format!("Failed to list PVCs in namespace '{}': {}", namespace, e))
        })?;

        let mut infos = Vec::new();
        for pvc in pvc_list {
            let name = pvc.name_any();
            let status = pvc.status.as_ref().and_then(|s| s.phase.clone()).unwrap_or_else(|| "Unknown".to_string());
            let volume = pvc.spec.as_ref().and_then(|s| s.volume_name.clone()).unwrap_or_else(|| "-".to_string());
    ...
    pub async fn get_persistent_volume_claim_details(
        ...
        api.get(name).await.map_err(|e| {
            AppError::ExternalService(format!("Failed to get PVC '{}' in namespace '{}': {}", name, namespace, e))
        })
    }
}
