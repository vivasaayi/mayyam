        ...
        match workflow {
            ResourceAnalysisWorkflow::Performance => {
                ...
                if let Some(hits_metric) = MetricsAnalyzer::find_metric(metrics, "CacheHits") {
                    if let Some(misses_metric) =
                        MetricsAnalyzer::find_metric(metrics, "CacheMisses")
                    {
                        let (hits_avg, _) =
                            MetricsAnalyzer::calculate_statistics(&hits_metric.datapoints);
                        let (misses_avg, _) =
                            MetricsAnalyzer::calculate_statistics(&misses_metric.datapoints);
                ...
                if let Some(evictions_metric) = MetricsAnalyzer::find_metric(metrics, "Evictions") {
                    let (avg, max) =
                        MetricsAnalyzer::calculate_statistics(&evictions_metric.datapoints);
                    analysis.push_str(&format!(
            ...
            ResourceAnalysisWorkflow::Memory => {
                ...
                if let Some(memory_metric) =
                    MetricsAnalyzer::find_metric(metrics, "DatabaseMemoryUsagePercentage")
                {
                    let (avg, max) =
                        MetricsAnalyzer::calculate_statistics(&memory_metric.datapoints);
                    analysis.push_str(&format!(
                        "## Memory Usage\n- Average: {:.2}%\n- Peak: {:.2}%\n\n",
                        avg, max
                    ));
