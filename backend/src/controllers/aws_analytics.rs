...
use crate::services::analytics::aws_analytics::models::analytics::{AwsResourceAnalysisRequest, AwsResourceAnalysisResponse, ResourceRelatedQuestionRequest};
...
impl AwsAnalyticsController {
    ...
    pub async fn analyze_resource_html(&self, req: web::Json<AwsResourceAnalysisRequest>) -> Result<impl Responder, AppError> {
    ...
    fn extract_recommendations_from_content(&self, content: &str) -> Vec<String> {
        content.lines()
            .filter(|line| line.to_lowercase().contains("recommend") || line.to_lowercase().contains("action"))
    ...
    pub fn configure(cfg: &mut web::ServiceConfig, controller: Arc<AwsAnalyticsController>) {
        ...
        cfg.service(
            web::scope("/api/aws/analytics")
                ...
                .route("/analyze/html", web::post().to(move |req: web::Json<AwsResourceAnalysisRequest>| {
