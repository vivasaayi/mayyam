...
use crate::services::aws::aws_types::kinesis::{
    KinesisPutRecordRequest, KinesisPutRecordsRequest, KinesisPutRecordsResponse,
    KinesisGetRecordsRequest, KinesisGetRecordsResponse, 
    KinesisGetShardIteratorRequest, KinesisGetShardIteratorResponse
};
...
impl KinesisDataPlane {
    ...
    pub async fn put_records(&self, aws_account_dto: &AwsAccountDto, request: &KinesisPutRecordsRequest) -> Result<KinesisPutRecordsResponse, AppError> {
        ...
        let stream_name = request.stream_name.as_ref()
            .ok_or_else(|| AppError::BadRequest("stream_name is required".to_string()))?;
            
        let response = client.put_records()
            .stream_name(stream_name)
            .set_records(Some(records))
            .send()
            .await
            .map_err(|e| AppError::ExternalService(format!("Failed to put records to Kinesis stream: {}", e)))?;
    ...
    pub async fn get_records(&self, aws_account_dto: &AwsAccountDto, request: &KinesisGetRecordsRequest) -> Result<KinesisGetRecordsResponse, AppError> {
        ...
        let response = get_records_request
            .send()
            .await
            .map_err(|e| AppError::ExternalService(format!("Failed to get records from Kinesis stream: {}", e)))?;
    ...
    pub async fn get_shard_iterator(&self, aws_account_dto: &AwsAccountDto, request: &KinesisGetShardIteratorRequest) -> Result<KinesisGetShardIteratorResponse, AppError> {
        ...
        let shard_iterator_type = match request.shard_iterator_type.as_str() {
            ...
            "AFTER_SEQUENCE_NUMBER" => aws_sdk_kinesis::types::ShardIteratorType::AfterSequenceNumber,
            "AT_TIMESTAMP" => aws_sdk_kinesis::types::ShardIteratorType::AtTimestamp,
            _ => return Err(AppError::BadRequest(format!("Invalid shard iterator type: {}", request.shard_iterator_type))),
        };
        
        let stream_name = request.stream_name.as_ref()
            .ok_or_else(|| AppError::BadRequest("stream_name is required".to_string()))?;
            
        let mut get_shard_iterator_request = client.get_shard_iterator()
            .stream_name(stream_name)
            .shard_id(&request.shard_id)
            .shard_iterator_type(shard_iterator_type);
            
        if let Some(sequence_number) = &request.starting_sequence_number {
            get_shard_iterator_request = get_shard_iterator_request.starting_sequence_number(sequence_number);
        }
        ...
        let response = get_shard_iterator_request
            .send()
            .await
            .map_err(|e| AppError::ExternalService(format!("Failed to get shard iterator: {}", e)))?;
