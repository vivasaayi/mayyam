    ...
    pub async fn analyze_ec2_instance(
        ...
        match workflow {
            ...
            ResourceAnalysisWorkflow::FiveWhy => {
                ...
                if let Some(cpu_metric) = MetricsAnalyzer::find_metric(metrics, "CPUUtilization") {
                    let (avg, max) = MetricsAnalyzer::calculate_statistics(&cpu_metric.datapoints);
                    analysis.push_str(&format!("- CPU Utilization: Avg {:.2}%, Max {:.2}%\n", avg, max));
                ...
                if let Some(mem_metric) = MetricsAnalyzer::find_metric(metrics, "MemoryUtilization") {
                    let (avg, max) = MetricsAnalyzer::calculate_statistics(&mem_metric.datapoints);
                    analysis.push_str(&format!("- Memory Utilization: Avg {:.2}%, Max {:.2}%\n", avg, max));
                ...
                analysis.push_str("To begin your root cause analysis, select one of the following questions:\n\n");
        ...
        if question.starts_with("why") {
            ...
            if question.contains("cpu") && question.contains("high") {
                ...
                if let Some(cpu_metric) = MetricsAnalyzer::find_metric(metrics, "CPUUtilization") {
                    ...
                    answer.push_str("- Consider enabling detailed monitoring for more granular data\n");
                }
            } else if question.contains("memory") && (question.contains("high") || question.contains("usage")) {
                ...
                if let Some(mem_metric) = MetricsAnalyzer::find_metric(metrics, "MemoryUtilization") {
                    ...
                    answer.push_str("### Next steps for investigation:\n\n");
                    answer.push_str("- Use tools like `free`, `vmstat`, or CloudWatch Memory Monitoring\n");
                ...
                answer.push_str("### Next steps for investigation:\n\n");
                answer.push_str("- Monitor disk I/O metrics using CloudWatch or tools like `iostat`\n");
                answer.push_str("- Check for processes with high disk activity\n");
                answer.push_str("- Consider upgrading to a different EBS volume type (e.g., gp3, io2)\n");
                ...
                if let Some(net_in) = MetricsAnalyzer::find_metric(metrics, "NetworkIn") {
                    let (avg_in, max_in) = MetricsAnalyzer::calculate_statistics(&net_in.datapoints);
                    if let Some(net_out) = MetricsAnalyzer::find_metric(metrics, "NetworkOut") {
                        let (avg_out, max_out) = MetricsAnalyzer::calculate_statistics(&net_out.datapoints);
                ...
                answer.push_str("### Potential root causes:\n\n");
                answer.push_str("1. **Network congestion**: High traffic periods or network bottlenecks.\n\n");
                answer.push_str("2. **Instance type limitations**: The instance type may have limited network bandwidth.\n\n");
                answer.push_str("3. **Application behavior**: Bursty application traffic patterns.\n\n");
                ...
                answer.push_str("### Potential root causes:\n\n");
                answer.push_str("1. **Resource contention**: Competition for shared resources on the host.\n\n");
                answer.push_str("2. **Burstable performance**: If using a burstable instance type (T2/T3), CPU credits may be depleting.\n\n");
                answer.push_str("3. **Workload variability**: Unpredictable or variable workload patterns.\n\n");
                ...
                answer.push_str("- Check for correlation between performance issues and specific events\n");
                ...
                if let Some(cpu_metric) = MetricsAnalyzer::find_metric(metrics, "CPUUtilization") {
                    let (avg, max) = MetricsAnalyzer::calculate_statistics(&cpu_metric.datapoints);
                    answer.push_str(&format!("- CPU Utilization: Avg {:.2}%, Max {:.2}%\n", avg, max));
                }

                if let Some(mem_metric) = MetricsAnalyzer::find_metric(metrics, "MemoryUtilization") {
                    let (avg, max) = MetricsAnalyzer::calculate_statistics(&mem_metric.datapoints);
                    answer.push_str(&format!("- Memory Utilization: Avg {:.2}%, Max {:.2}%\n", avg, max));
                }

                answer.push_str("\n### General Troubleshooting Approach:\n\n");
                answer.push_str("1. **Identify symptoms**: Collect data on when and how the issue occurs\n");
                answer.push_str("2. **Isolate variables**: Determine what factors correlate with the issue\n");
                answer.push_str("3. **Test hypotheses**: Make controlled changes to verify root causes\n");
                answer.push_str("4. **Implement solutions**: Address the underlying issues\n");
                answer.push_str("5. **Monitor results**: Verify that the solutions resolve the problem\n\n");
            ...
            if let Some(instance_type) = resource.resource_data.get("instance_type").and_then(|v| v.as_str()) {
