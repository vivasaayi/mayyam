...
pub async fn s3_get_object(
    ...
    let response = aws_data_plane.s3_get_object(profile_opt, region, &request).await?;
...
pub async fn s3_put_object(
    ...
    let response = aws_data_plane.s3_put_object(profile_opt, &region, &req).await?;
...
pub async fn dynamodb_get_item(
    ...
    let response = aws_data_plane.dynamodb_get_item(profile_opt, &region, &request).await?;
...
pub async fn dynamodb_put_item(
    ...
    let response = aws_data_plane.dynamodb_put_item(profile_opt, &region, &request).await?;
...
pub async fn dynamodb_query(
    ...
    let response = aws_data_plane.dynamodb_query(profile_opt, &region, &request).await?;
...
pub async fn sqs_send_message(
    ...
    let response = aws_data_plane.sqs_send_message(profile_opt, &region, &req).await?;
...
pub async fn sqs_receive_messages(
    ...
    let response = aws_data_plane.sqs_receive_messages(profile_opt, &region, &req).await?;
...
pub async fn kinesis_put_record(
    ...
    let response = aws_data_plane.kinesis_put_record(profile_opt, &region, &req).await?;
...
pub async fn get_cloudwatch_logs(
    ...
    let result = cloudwatch_service.get_logs(profile_opt, &region, &request).await?;
...
pub async fn schedule_metrics_collection(
    ...
    // Export options
    let export_path = body.get("export_path")
        .and_then(|v| v.as_str())
        .map(String::from);
    
    let upload_to_s3 = body.get("upload_to_s3")
        .and_then(|v| v.as_bool());
    
    let s3_bucket = body.get("s3_bucket")
        .and_then(|v| v.as_str())
        .map(String::from);
    
    let post_to_url = body.get("post_to_url")
        .and_then(|v| v.as_str())
        .map(String::from);
    ...
    let request = CloudWatchMetricsRequest {
        ...
        resource_type,
        metrics,
        start_time: now - chrono::Duration::minutes(10), // Last 10 minutes (placeholder)
        end_time: now,
        period,
        export_path,
        upload_to_s3,
    ...
    let job_id = cloudwatch_service.schedule_metrics_collection(&request, interval_seconds).await?;
