...
pub async fn list_pdb_controller(
    claims: web::ReqData<Claims>,
    db: web::Data<Arc<DatabaseConnection>>,
    path: web::Path<(String, String)>,
    svc: web::Data<Arc<PodDisruptionBudgetsService>>,
) -> Result<impl Responder, AppError> {
    let (cluster_id, ns) = path.into_inner();
    debug!(target: "mayyam::controllers::pdb", user_id = %claims.username, %cluster_id, %ns, "List PDB");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
    let list = svc.list(&cfg, &ns).await?;
    Ok(HttpResponse::Ok().json(list))
}

pub async fn get_pdb_controller(
    claims: web::ReqData<Claims>,
    db: web::Data<Arc<DatabaseConnection>>,
    ...
    debug!(target: "mayyam::controllers::pdb", user_id = %claims.username, %cluster_id, %ns, %name, "Get PDB");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
    let item = svc.get(&cfg, &ns, &name).await?;
    Ok(HttpResponse::Ok().json(item))
}

pub async fn upsert_pdb_controller(
    claims: web::ReqData<Claims>,
    db: web::Data<Arc<DatabaseConnection>>,
    path: web::Path<(String, String, String)>,
    body: web::Json<PodDisruptionBudget>,
    svc: web::Data<Arc<PodDisruptionBudgetsService>>,
) -> Result<impl Responder, AppError> {
    let (cluster_id, ns, name) = path.into_inner();
    let mut item = body.into_inner();
    if item.metadata.name.as_deref() != Some(&name) { item.metadata.name = Some(name.clone()); }
    debug!(target: "mayyam::controllers::pdb", user_id = %claims.username, %cluster_id, %ns, %name, "Upsert PDB");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
    let updated = svc.upsert(&cfg, &ns, &item).await?;
    Ok(HttpResponse::Ok().json(updated))
}

pub async fn delete_pdb_controller(
    claims: web::ReqData<Claims>,
    db: web::Data<Arc<DatabaseConnection>>,
    path: web::Path<(String, String, String)>,
    svc: web::Data<Arc<PodDisruptionBudgetsService>>,
    ...
    let (cluster_id, ns, name) = path.into_inner();
    debug!(target: "mayyam::controllers::pdb", user_id = %claims.username, %cluster_id, %ns, %name, "Delete PDB");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
