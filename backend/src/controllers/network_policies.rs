...
pub async fn list_network_policies_controller(
    claims: web::ReqData<Claims>,
    db: web::Data<Arc<DatabaseConnection>>,
    path: web::Path<(String, String)>,
    svc: web::Data<Arc<NetworkPoliciesService>>,
) -> Result<impl Responder, AppError> {
    let (cluster_id, ns) = path.into_inner();
    debug!(target: "mayyam::controllers::network_policies", user_id = %claims.username, %cluster_id, %ns, "List NetworkPolicies");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
    let list = svc.list(&cfg, &ns).await?;
    Ok(HttpResponse::Ok().json(list))
}

pub async fn get_network_policy_controller(
    claims: web::ReqData<Claims>,
    db: web::Data<Arc<DatabaseConnection>>,
    ...
    debug!(target: "mayyam::controllers::network_policies", user_id = %claims.username, %cluster_id, %ns, %name, "Get NetworkPolicy");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
    let item = svc.get(&cfg, &ns, &name).await?;
    Ok(HttpResponse::Ok().json(item))
}

pub async fn upsert_network_policy_controller(
    claims: web::ReqData<Claims>,
    db: web::Data<Arc<DatabaseConnection>>,
    path: web::Path<(String, String, String)>,
    body: web::Json<NetworkPolicy>,
    svc: web::Data<Arc<NetworkPoliciesService>>,
) -> Result<impl Responder, AppError> {
    let (cluster_id, ns, name) = path.into_inner();
    let mut item = body.into_inner();
    if item.metadata.name.as_deref() != Some(&name) { item.metadata.name = Some(name.clone()); }
    debug!(target: "mayyam::controllers::network_policies", user_id = %claims.username, %cluster_id, %ns, %name, "Upsert NetworkPolicy");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
    let updated = svc.upsert(&cfg, &ns, &item).await?;
    Ok(HttpResponse::Ok().json(updated))
}

pub async fn delete_network_policy_controller(
    claims: web::ReqData<Claims>,
    db: web::Data<Arc<DatabaseConnection>>,
    path: web::Path<(String, String, String)>,
    svc: web::Data<Arc<NetworkPoliciesService>>,
    ...
    let (cluster_id, ns, name) = path.into_inner();
    debug!(target: "mayyam::controllers::network_policies", user_id = %claims.username, %cluster_id, %ns, %name, "Delete NetworkPolicy");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
