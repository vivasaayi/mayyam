...
async fn test_aws_account_crud() {
    ...
    if config.database.postgres.is_empty() {
        if let Ok(host) = std::env::var("POSTGRES_HOST") {
            ...
            let port: u16 = std::env::var("POSTGRES_PORT").ok().and_then(|s| s.parse().ok()).unwrap_or(5432);
            config.database.postgres.push(crate::config::PostgresConfig{
                name: "test".into(), host, port, username: user, password: pass, database: dbname, ssl_mode: None
            });
        }
    }
