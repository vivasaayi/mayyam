    ...
    pub async fn sync_buckets(
        ...
        for aws_bucket in response.buckets() {
            ...
            let location_resp = client
                ...
                .map_err(|e| {
                    AppError::ExternalService(format!(
                        "Failed to get bucket location for {}: {}",
                        bucket_name, e
                    ))
                })?;

            let bucket_region = location_resp
                .location_constraint()
                .map(|c| c.as_str())
                .unwrap_or("FIX_ME");
            ...
            let versioning_resp = client
                ...
                .map_err(|e| {
                    AppError::ExternalService(format!(
                        "Failed to get versioning for {}: {}",
                        bucket_name, e
                    ))
                })?;

            let versioning_enabled = versioning_resp
                .status()
                ...
                Ok(lifecycle_resp) => {
                    for rule in lifecycle_resp.rules() {
                        ...
                        for transition in rule.transitions() {
                            ...
                            rule_data
                                .insert("transition_days".to_string(), json!(transition.days()));

                            if let Some(storage_class) =
                                transition.storage_class().map(|s| s.as_str())
                            {
