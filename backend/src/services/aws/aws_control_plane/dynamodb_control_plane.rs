...
use crate::services::aws::aws_types::dynamodb::{
    DynamoDbAttributeDefinition, DynamoDbKeySchema, DynamoDbProvisionedThroughput,
    DynamoDbTableInfo,
};
...
impl DynamoDbControlPlane {
    ...
    pub async fn sync_tables(
        ...
        let list_response = client.list_tables().send().await.map_err(|e| {
        ...
        debug!(
            "Fetched {} DynamoDB tables",
            list_response.table_names().len()
        );
            ...
            if let Some(table_details) = describe_resp.table() {
                ...
                if let Some(creation_date) = table_details.creation_date_time() {
                    resource_data.insert(
                        ...
                        if let Ok(formatted_date) =
                            creation_date.fmt(aws_smithy_types::date_time::Format::DateTime)
                        {
                            json!(formatted_date)
                        } else {
                ...
                if let Some(throughput) = table_details.provisioned_throughput() {
                    ...
                    resource_data
                        .insert("provisioned_throughput".to_string(), json!(throughput_data));
                }
                ...
                for key in table_details.key_schema() {
                    ...
                    key_data.insert("key_type".to_string(), json!(key.key_type().as_str()));
                ...
                for attr in table_details.attribute_definitions() {
                    ...
                    attr_map.insert(
                        ...
                        json!(attr.attribute_type().as_str()),
                    );
    ...
    pub async fn list_tables(
        ...
        let response = request.send().await.map_err(|e| {
            AppError::ExternalService(format!("Failed to list DynamoDB tables: {}", e))
        })?;
    ...
    pub async fn describe_table(
        ...
        aws_account_dto: AwsAccountDto,
        table_name: &str,
    ) -> Result<DynamoDbTableInfo, AppError> {
        let client = self
            .aws_service
            .create_dynamodb_client(&aws_account_dto)
            .await?;
        ...
        let table_details = response.table().ok_or_else(|| {
            AppError::ExternalService(format!("No table details returned for {}", table_name))
        })?;
    ...
    pub async fn create_table(
        ...
        // Convert key schema
        let aws_key_schema: Vec<aws_sdk_dynamodb::types::KeySchemaElement> = key_schema
            .iter()
            .map(|key| {
        ...
        let aws_attr_defs: Vec<aws_sdk_dynamodb::types::AttributeDefinition> =
            attribute_definitions
                ...
                .map_err(|e| {
                    AppError::ExternalService(format!(
                        "Failed to build attribute definitions: {}",
                        e
                    ))
                })?;
        ...
        let aws_throughput = aws_sdk_dynamodb::types::ProvisionedThroughput::builder()
            ...
            .build()
            .map_err(|e| {
                AppError::ExternalService(format!("Failed to build provisioned throughput: {}", e))
            })?;
        ...
        // Extract table details from response
        let table_details = response.table_description().ok_or_else(|| {
            AppError::ExternalService(format!(
                "No table details returned for newly created table {}",
                table_name
            ))
        })?;
    ...
    pub async fn update_table(
        ...
        aws_account_dto: AwsAccountDto,
        table_name: &str,
        provisioned_throughput: Option<DynamoDbProvisionedThroughput>,
    ) -> Result<DynamoDbTableInfo, AppError> {
        let client = self
            .aws_service
            .create_dynamodb_client(&aws_account_dto)
        ...
        if let Some(throughput) = &provisioned_throughput {
            let aws_throughput = aws_sdk_dynamodb::types::ProvisionedThroughput::builder()
                ...
                .map_err(|e| {
                    AppError::ExternalService(format!(
                        "Failed to build provisioned throughput: {}",
                        e
                    ))
                })?;
        ...
        let response = request.send().await.map_err(|e| {
            AppError::ExternalService(format!(
                "Failed to update DynamoDB table {}: {}",
                table_name, e
            ))
        })?;
        ...
        let table_details = response.table_description().ok_or_else(|| {
            AppError::ExternalService(format!(
                "No table details returned for updated table {}",
                table_name
            ))
        })?;
        ...
        let describe_resp = client
            ...
            .map_err(|e| {
                AppError::ExternalService(format!(
                    "Failed to describe updated table {}: {}",
                    table_name, e
                ))
            })?;

        let table_desc = describe_resp.table().ok_or_else(|| {
            AppError::ExternalService(format!("No details found for table {}", table_name))
        })?;
