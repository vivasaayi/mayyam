...
impl CronJobsService {
    ...
    async fn api(cluster: &KubernetesClusterConfig, namespace: &str) -> Result<Api<CronJob>, AppError> {
    ...
    pub async fn list(&self, cluster: &KubernetesClusterConfig, namespace: &str) -> Result<Vec<CronJob>, AppError> {
    ...
    pub async fn get(&self, cluster: &KubernetesClusterConfig, namespace: &str, name: &str) -> Result<CronJob, AppError> {
        let api: Api<CronJob> = Api::namespaced(ClientFactory::get_client(cluster).await?, namespace);
        api.get(name).await.map_err(|e| AppError::Kubernetes(e.to_string()))
    }

    pub async fn upsert(&self, cluster: &KubernetesClusterConfig, namespace: &str, item: &CronJob) -> Result<CronJob, AppError> {
        let api: Api<CronJob> = Api::namespaced(ClientFactory::get_client(cluster).await?, namespace);
        let params = PatchParams::apply("mayyam").force();
        api.patch(item.metadata.name.as_ref().ok_or_else(|| AppError::BadRequest("CronJob.metadata.name required".into()))?, &params, &Patch::Apply(item))
            .await
