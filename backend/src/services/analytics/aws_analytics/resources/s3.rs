        ...
        match workflow {
            ResourceAnalysisWorkflow::Cost => {
                ...
                if let Some(size_metric) = MetricsAnalyzer::find_metric(metrics, "BucketSizeBytes") {
                    ...
                    analysis.push_str(&format!("- Storage Cost per GB: ${:.3}/month\n", monthly_cost_per_gb));
                    analysis.push_str(&format!("- Estimated Monthly Storage Cost: ${:.2}\n\n", monthly_cost));
                    ...
                    analysis.push_str(&format!("   - Archive older data to Glacier (${:.3}/GB/month)\n", monthly_cost_per_gb * 0.2));
                    analysis.push_str(&format!("   - Potential savings: up to ${:.2}/month\n\n", monthly_cost * 0.7));
                    
                    // Object expiration recommendations
                    analysis.push_str("2. **Set Up Object Expiration**\n");
                    analysis.push_str("   - Automatically delete objects that are no longer needed\n");
                    analysis.push_str("   - Consider setting expiration rules for temporary data\n\n");
                    ...
                    analysis.push_str("   - Use compression for text files, logs, and other compressible data\n");
                    analysis.push_str("   - Potential space savings: 40-60% for text-based files\n");
                }
                
                // Request analysis
                if let Some(requests_metric) = MetricsAnalyzer::find_metric(metrics, "AllRequests") {
                    let (avg, _) = MetricsAnalyzer::calculate_statistics(&requests_metric.datapoints);
                    ...
                    analysis.push_str(&format!("- Estimated Monthly Requests: {:.0}\n", requests_per_month));
                    analysis.push_str(&format!("- Estimated GET Request Cost: ${:.2}\n", get_cost));
                    analysis.push_str(&format!("- Estimated PUT/POST/LIST Request Cost: ${:.2}\n", put_cost));
                    analysis.push_str(&format!("- Total Request Cost: ${:.2}\n\n", total_request_cost));
            ...
            ResourceAnalysisWorkflow::Storage => {
                ...
                if let Some(count_metric) = MetricsAnalyzer::find_metric(metrics, "NumberOfObjects") {
                    ...
                    if let Some(size_metric) = MetricsAnalyzer::find_metric(metrics, "BucketSizeBytes") {
                        let (size_avg, _) = MetricsAnalyzer::calculate_statistics(&size_metric.datapoints);
                        if avg > 0.0 {
                            let avg_object_size = size_avg / avg;
                            
                            analysis.push_str(&format!("## Object Size Distribution\n"));
                            analysis.push_str(&format!("- Average Object Size: {:.2} KB\n\n", avg_object_size / 1024.0));
                            
                            // Add recommendations based on average object size
                            if avg_object_size < 128.0 * 1024.0 {  // Less than 128KB
                                analysis.push_str("⚠️ **Small Object Size Detected**\n");
                                analysis.push_str("Consider:\n");
                                analysis.push_str("1. Batching small objects into larger archives\n");
                                analysis.push_str("2. Using S3 Inventory to analyze object size distribution\n");
                                analysis.push_str("3. Implementing object compression for text-based files\n\n");
                            }
                        }
                    }
                }
                ...
                analysis.push_str("4. **Intelligent-Tiering**: For data with changing access patterns\n\n");
                
                analysis.push_str("## Storage Optimization Recommendations\n");
                analysis.push_str("1. **Implement Lifecycle Policies**\n");
                analysis.push_str("   - Automatically transition objects between storage classes\n");
