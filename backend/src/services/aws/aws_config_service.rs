...
impl AwsConfigService {
    ...
    pub async fn get_aws_config(&self, region: &str, aws_account_dto: &AwsAccountDto) -> Result<aws_config::SdkConfig, AppError> {
        ...
        if let Some(profile) = aws_account_dto.profile.as_ref() {
            ...
            builder = builder.credentials_provider(
                Credentials::builder()
                    .access_key_id(aws_account_dto.access_key_id.as_ref().unwrap().clone().as_str())
                    .secret_access_key(aws_account_dto.secret_access_key.as_ref().unwrap().clone().as_str())
                    .build()
            );
        }
