...
use crate::services::llm::interface::{ResponseMetadata, TokenUsage, UnifiedLlmResponse};
...
impl ResponseFormatter {
    ...
    fn extract_code_blocks_from_text(
        &self,
        text: &str,
    ) -> Result<(String, Vec<CodeBlock>), AppError> {
    ...
    fn extract_structured_data_from_text(
        &self,
        text: &str,
    ) -> Result<(String, Vec<StructuredData>), AppError> {
    ...
    fn normalize_whitespace_in_text(&self, text: &str) -> String {
        ...
        let result = regex::Regex::new(r" +").unwrap().replace_all(text, " ");
...
impl ResponseAnalyzer {
    ...
    fn calculate_token_efficiency(response: &UnifiedLlmResponse) -> Option<f64> {
        if let (Some(total), Some(completion)) = (
