...
use kube::{config::{Config as KubeConfig, KubeConfigOptions, Kubeconfig}, Client};
...
static CLIENT_CACHE: Lazy<dashmap::DashMap<u64, Arc<Client>>> = Lazy::new(|| dashmap::DashMap::new());
...
fn b64_if_needed(data_opt: &Option<String>) -> Option<String> {
    match data_opt {
        ...
        Some(s) => {
            ...
            if is_b64 { Some(s.clone()) } else { Some(BASE64.encode(s.as_bytes())) }
        }
        None => None,
    }
}
    ...
    async fn build_client(cluster_config: &KubernetesClusterConfig) -> Result<Client, AppError> {
        ...
        if let Some(path) = &cluster_config.kube_config_path {
            ...
            let kubeconfig = Kubeconfig::read_from(path)
                .map_err(|e| AppError::ExternalService(format!("Failed to read kubeconfig from path: {}", e)))?;
            ...
            .await
            .map_err(|e| AppError::ExternalService(format!("Failed to create Kubernetes client config: {}", e)))?;
            return Client::try_from(cfg)
                .map_err(|e| AppError::ExternalService(format!("Failed to create Kubernetes client: {}", e)));
        }
        ...
        if let Some(server) = &cluster_config.api_server_url {
            ...
            named_cluster.cluster = Some(kube::config::Cluster {
                ...
                certificate_authority_data: b64_if_needed(&cluster_config.certificate_authority_data),
                ..Default::default()
            });
            ...
            if let Some(token) = &cluster_config.token { auth.token = Some(Secret::new(token.clone())); }
            ...
            .await
            .map_err(|e| AppError::ExternalService(format!("Failed to create Kubernetes config from direct settings: {}", e)))?;
        ...
        info!(target = "mayyam::k8s::client", "Inferring Kubernetes config (no kubeconfig path or server provided)");
        let infer_config = kube::Config::infer().await
            .map_err(|e| AppError::ExternalService(format!("Failed to infer Kubernetes config: {}", e)))?;
        Client::try_from(infer_config)
            .map_err(|e| AppError::ExternalService(format!("Failed to create Kubernetes client from inferred config: {}", e)))
    }
}
