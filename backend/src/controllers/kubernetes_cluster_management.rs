...
use crate::middleware::auth::Claims; // For accessing user_id from token

#[derive(Clone)]
pub struct KubernetesClusterManagementController { // Renamed from ClusterManagementController
    cluster_repo: Arc<ClusterRepository>,
}

#[derive(Deserialize, Debug)]
pub struct KubernetesClusterQuery { // Renamed from ClusterQuery
...
impl KubernetesClusterManagementController { // Renamed from ClusterManagementController
    ...
    pub async fn create_kubernetes_cluster(
        ...
        debug!("Attempting to create Kubernetes cluster with name: {} by user: {}", req.name, claims.sub); // Use claims.sub for user identifier
        match self.cluster_repo.create_kubernetes_cluster(&req.into_inner(), &claims.sub.to_string()).await { // Pass claims.sub as string
            Ok(cluster) => {
                info!("Successfully created Kubernetes cluster with ID: {}", cluster.id);
    ...
    pub async fn get_all_kubernetes_clusters(&self, _query: web::Query<KubernetesClusterQuery>) -> impl Responder { 
        debug!("Fetching all Kubernetes clusters.");
        match self.cluster_repo.find_by_type("kubernetes").await {
            Ok(clusters) => {
                info!("Successfully fetched {} Kubernetes clusters", clusters.len());
        ...
        match self.cluster_repo.find_by_id(cluster_id).await {
            Ok(Some(cluster)) => {
                if cluster.cluster_type == "kubernetes" {
                    info!("Successfully fetched Kubernetes cluster with ID: {}", cluster.id);
                    HttpResponse::Ok().json(cluster)
                } else {
                    error!("Cluster with ID: {} is not a Kubernetes cluster", cluster_id);
                    HttpResponse::NotFound().json(serde_json::json!({ "error": "Kubernetes cluster not found" }))
                }
            }
            Ok(None) => {
    ...
    pub async fn update_kubernetes_cluster(
        ...
        debug!("Attempting to update Kubernetes cluster with ID: {}", cluster_id);
        ...
        match self.cluster_repo.update(cluster_id, &update_req.name, config_value).await {
            Ok(cluster) => {
    ...
    pub async fn delete_kubernetes_cluster(&self, claims: web::ReqData<Claims>, path: web::Path<Uuid>) -> impl Responder {
        let cluster_id = path.into_inner();
        debug!("User {} attempting to delete Kubernetes cluster with ID: {}", claims.username, cluster_id);
        ...
        match self.cluster_repo.delete(cluster_id).await { // Consider adding a type check in repo or here
            Ok(_) => {
                info!("Successfully deleted Kubernetes cluster with ID: {}", cluster_id);
