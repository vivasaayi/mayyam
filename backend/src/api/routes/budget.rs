...
pub fn configure_routes(cfg: &mut web::ServiceConfig, cost_budget_repo: Arc<CostBudgetRepository>) {
    ...
    cfg.service(
        web::scope("/api/budgets")
            ...
            .route(
                ...
                web::get().to(budget::get_budget),
            )
            .route(
                "/{account_id}/{budget_id}",
                web::put().to(budget::update_budget),
            )
            .route(
                "/{account_id}/{budget_id}",
                web::delete().to(budget::delete_budget),
            )
            .route(
                "/{account_id}/{budget_id}/status",
                web::get().to(budget::get_budget_status),
            )
            .route(
                "/{account_id}/alerts",
                web::get().to(budget::get_budget_alerts),
            )
