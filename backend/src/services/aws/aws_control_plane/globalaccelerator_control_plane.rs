...
impl GlobalAcceleratorControlPlane {
    ...
    pub async fn sync_accelerators(
        ...
        let client = self.aws_service.create_globalaccelerator_client(aws_account_dto).await?;
        let mut resources: Vec<AwsResourceModel> = Vec::new();

        // List Global Accelerators from AWS
        let response = client.list_accelerators()
            .send()
            .await
            .map_err(|e| {
