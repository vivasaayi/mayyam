...
use aws_sdk_cloudwatchlogs::{
    operation::filter_log_events::FilterLogEventsInput as FilterLogEventsRequest, types::OrderBy,
...
impl CloudWatchLogs for CloudWatchService {
    async fn get_logs(
        ...
        let response = client
            ...
            .await
            .map_err(|e| {
                AppError::ExternalService(format!("Failed to get CloudWatch logs: {}", e))
            })?;
    ...
    async fn get_filtered_logs(
        ...
        loop {
            ...
            let response = current_req.send().await.map_err(|e| {
                AppError::ExternalService(format!("Failed to get CloudWatch logs: {}", e))
            })?;
