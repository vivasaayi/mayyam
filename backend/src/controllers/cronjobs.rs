...
pub async fn list_cronjobs_controller(
    ...
    debug!(target: "mayyam::controllers::cronjobs", user_id = %claims.username, %cluster_id, %ns, "List CronJobs");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
    let list = svc.list(&cfg, &ns).await?;
    Ok(HttpResponse::Ok().json(list))
}

pub async fn get_cronjob_controller(
    claims: web::ReqData<Claims>,
    db: web::Data<Arc<DatabaseConnection>>,
    ...
    debug!(target: "mayyam::controllers::cronjobs", user_id = %claims.username, %cluster_id, %ns, %name, "Get CronJob");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
    let item = svc.get(&cfg, &ns, &name).await?;
    Ok(HttpResponse::Ok().json(item))
}

pub async fn upsert_cronjob_controller(
    claims: web::ReqData<Claims>,
    db: web::Data<Arc<DatabaseConnection>>,
    path: web::Path<(String, String, String)>,
    body: web::Json<CronJob>,
    svc: web::Data<Arc<CronJobsService>>,
) -> Result<impl Responder, AppError> {
    let (cluster_id, ns, name) = path.into_inner();
    let mut item = body.into_inner();
    if item.metadata.name.as_deref() != Some(&name) { item.metadata.name = Some(name.clone()); }
    debug!(target: "mayyam::controllers::cronjobs", user_id = %claims.username, %cluster_id, %ns, %name, "Upsert CronJob");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
    let updated = svc.upsert(&cfg, &ns, &item).await?;
    Ok(HttpResponse::Ok().json(updated))
}

pub async fn delete_cronjob_controller(
    claims: web::ReqData<Claims>,
    db: web::Data<Arc<DatabaseConnection>>,
    path: web::Path<(String, String, String)>,
    svc: web::Data<Arc<CronJobsService>>,
    ...
    let (cluster_id, ns, name) = path.into_inner();
    debug!(target: "mayyam::controllers::cronjobs", user_id = %claims.username, %cluster_id, %ns, %name, "Delete CronJob");
    let cfg = get_cluster_config_by_id(db.get_ref().as_ref(), &cluster_id).await?;
