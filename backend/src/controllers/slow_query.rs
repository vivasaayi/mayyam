...
pub async fn get_slow_queries(
    ...
    let start_time = if let Some(start_str) = &query.start_time {
        Some(
            NaiveDateTime::parse_from_str(start_str, "%Y-%m-%dT%H:%M:%S%.fZ")
    ...
    let end_time = if let Some(end_str) = &query.end_time {
        Some(
            NaiveDateTime::parse_from_str(end_str, "%Y-%m-%dT%H:%M:%S%.fZ")
    ...
    let events =
        if let (Some(cluster_id), Some(start), Some(end)) = (cluster_id, start_time, end_time) {
            controller
                .slow_query_repo
                .find_by_cluster_and_time_range(cluster_id, start, end)
                .await?
        } else {
            ...
            if let Some(cluster_id) = cluster_id {
                ...
                controller
                    .slow_query_repo
                    .find_top_by_total_time(None, 24, limit as u64)
                    .await?
            }
        };
...
pub async fn get_slow_query_stats(
    ...
    let events_last_hour = controller
        .slow_query_repo
        .find_top_by_total_time(cluster_id, 1, 1000)
        .await?
        .len() as u64;

    // Get events in last 24 hours
    let events_last_24h = controller
        .slow_query_repo
        .find_top_by_total_time(cluster_id, 24, 1000)
        .await?
        .len() as u64;

    // Get top slow queries for analysis
    let top_slow_queries = controller
        .slow_query_repo
        .find_top_by_total_time(cluster_id, 24, 10)
        .await?;
    ...
    let max_query_time = top_slow_queries
        .iter()
        .map(|e| e.query_time)
...
pub async fn ingest_slow_queries(
    ...
    let cluster_id = Uuid::parse_str(&payload.cluster_id)
        .map_err(|e| AppError::BadRequest(format!("Invalid cluster UUID: {}", e)))?;
