...
pub async fn list_roles_controller(
    claims: web::ReqData<Claims>, db: web::Data<Arc<DatabaseConnection>>, path: web::Path<(String, String)>, svc: web::Data<Arc<RbacService>>,
...
pub async fn get_role_controller(
    claims: web::ReqData<Claims>, db: web::Data<Arc<DatabaseConnection>>, path: web::Path<(String, String, String)>, svc: web::Data<Arc<RbacService>>,
) -> Result<impl Responder, AppError> {
...
pub async fn upsert_role_controller(
    claims: web::ReqData<Claims>, db: web::Data<Arc<DatabaseConnection>>, path: web::Path<(String, String, String)>, body: web::Json<Role>, svc: web::Data<Arc<RbacService>>,
) -> Result<impl Responder, AppError> {
    let (cluster_id, ns, name) = path.into_inner();
    let mut item = body.into_inner();
    if item.metadata.name.as_deref() != Some(&name) { item.metadata.name = Some(name.clone()); }
...
pub async fn upsert_role_binding_controller(
    claims: web::ReqData<Claims>, db: web::Data<Arc<DatabaseConnection>>, path: web::Path<(String, String, String)>, body: web::Json<RoleBinding>, svc: web::Data<Arc<RbacService>>,
) -> Result<impl Responder, AppError> {
    let (cluster_id, ns, name) = path.into_inner();
    let mut item = body.into_inner();
    if item.metadata.name.as_deref() != Some(&name) { item.metadata.name = Some(name.clone()); }
...
pub async fn list_cluster_roles_controller(
    claims: web::ReqData<Claims>, db: web::Data<Arc<DatabaseConnection>>, path: web::Path<String>, svc: web::Data<Arc<RbacService>>,
) -> Result<impl Responder, AppError> {
...
pub async fn upsert_cluster_role_controller(
    claims: web::ReqData<Claims>, db: web::Data<Arc<DatabaseConnection>>, path: web::Path<(String, String)>, body: web::Json<ClusterRole>, svc: web::Data<Arc<RbacService>>,
) -> Result<impl Responder, AppError> {
    let (cluster_id, name) = path.into_inner();
    let mut item = body.into_inner();
    if item.metadata.name.as_deref() != Some(&name) { item.metadata.name = Some(name.clone()); }
...
pub async fn upsert_cluster_role_binding_controller(
    claims: web::ReqData<Claims>, db: web::Data<Arc<DatabaseConnection>>, path: web::Path<(String, String)>, body: web::Json<ClusterRoleBinding>, svc: web::Data<Arc<RbacService>>,
) -> Result<impl Responder, AppError> {
    let (cluster_id, name) = path.into_inner();
    let mut item = body.into_inner();
    if item.metadata.name.as_deref() != Some(&name) { item.metadata.name = Some(name.clone()); }
